...
/*  DynamicJsonBuffer jsonBuffer;
  ...
  //String newSendFuffer = "{\"hoseButton\":\"{0}\",\"irrigationButton\":\"{1}\",\"tank0Level0\":\"{2}\",\"tank0Level1\":\"{3}\",\"tank0Level2\":\"{4}\",\"tank0Level3\":\"{5}\",\"tank0Level4\":\"{6}\",\"tank1Level0\":\"{7}\",\"tank1Level1\":\"{8}\",\"tank1Level2\":\"{9}\",\"tank1Level3\":\"{10}\",\"tank1Level4\":\"{11}\"}";
  String newSendFuffer = "{'hoseButton':{0},'irrigationButton':'{1}','tank0Level0':'{2}','tank0Level1':'{3}','tank0Level2':'{4}','tank0Level3':'{5}','tank0Level4':'{6}','tank1Level0':'{7}','tank1Level1':'{8}','tank1Level2':'{9}','tank1Level3':'{10}','tank1Level4':'{11}'}";
  
  newSendFuffer.replace("{0}",String(hoseButtonPressed));
  newSendFuffer.replace("{1}",String(irrigationButtonPressed));

  newSendFuffer.replace("{2}",String(tankLevelBuffer[0]));
  newSendFuffer.replace("{3}",String(tankLevelBuffer[1]));
  newSendFuffer.replace("{4}",String(tankLevelBuffer[2]));
  newSendFuffer.replace("{5}",String(tankLevelBuffer[3]));
  newSendFuffer.replace("{6}",String(tankLevelBuffer[4]));

  newSendFuffer.replace("{7}",String(tankLevelBuffer[5]));
  newSendFuffer.replace("{8}",String(tankLevelBuffer[6]));
  newSendFuffer.replace("{9}",String(tankLevelBuffer[7]));
  newSendFuffer.replace("{10}",String(tankLevelBuffer[8]));
  newSendFuffer.replace("{11}",String(tankLevelBuffer[9]));
  
  //Serial.println("l: " + lastSendBuffer);
  //Serial.println("n: " + newSendFuffer);
    
  if (lastSendBuffer.equals(newSendFuffer) == false or loopCount > 10) {
    Serial.println(newSendFuffer);
    //root.printTo(Serial1);
    Serial1.print(newSendFuffer);
    Serial1.println("");
    Serial1.flush();
